...
switch($_GET[PHPIdSession]){
  ...
  default:
    echo"<div id=headermodul>Kartu Hasil Studi (KHS)</div>
          <form method=post  action='?page=levelakademikkhsmhs&PHPIdSession=CariMahasiswa'>
  ...
  if ($ab > 0){
  echo" <div id=headermodul>Kartu Hasil Studi (KHS)</div>";
  echo" <form  method=post  action='?page=levelakademikkhsmhs&PHPIdSession=CariMahasiswa'>
        <div id=groupmodul1><table id=tablemodul>
  ...
  echo"     </td></tr>
        ...
        <a class=s href='khsprint.php?tahun=$_POST[tahun]&NIM=$_POST[NIM]' target='_new'><img src=../img/printer.GIF> Cetak KHS |</a>";
  ...
  $sql="SELECT * FROM krs1 WHERE tahun='$_POST[tahun]' AND NIM='$_POST[NIM]'";
  $no=0;
  $qry=mysql_query($sql)
  or die ();
