...
switch($_GET[PHPIdSession]){
  ...
  echo" <div id=headermodul>Index Prestasi Kumulatif</div>
        <form method=post  action='?page=mahasiswatranskrip&PHPIdSession=CariMahasiswa'>
  ...
  echo" <div id=headermodul>Daftar Matakuliah Yang Telah Diambil</div>
        <form name=form1 method=post action=?page=mahasiswatranskrip&PHPIdSession=CariMahasiswa>
        <div id=groupmodul1><table id=tablemodul>    
        <tr><td class=cc>NIM</td>          <td class=cb><input type=text name=NIM value='$_SESSION[username]'></td>           
  ...
  echo"     </td></tr>
        ...
        <a class=s href='transkripprint.php?NIM=$_SESSION[username]&prodi=$data[kode_jurusan]' target='_new'><img src=../img/printer.GIF> Cetak Transkrip |</a>";
  ...
  $no++;
        $sqlr="SELECT * FROM view_ipk  WHERE  NIM='$_SESSION[username]' AND kode_mtk='$data[Kode_mtk]'";
