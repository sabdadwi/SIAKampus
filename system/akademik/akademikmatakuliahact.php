...
if ($page=='akademikmatakuliah' AND $act=='upkrklmmk'){
        $kode= $_REQUEST['mkiq'];
        $jur= $_REQUEST['mkjkq'];
 ...
 header('location:../media.php?page=akademikmatakuliah&PHPIdSession=Daftarkrklm&codd='.$kode.'&kode='.$jur);
}


elseif ($page=='akademikmatakuliah' AND $act=='delprog'){
        $kode= $_REQUEST['mkiq'];
        $jur= $_REQUEST['mkjkq'];

        mysql_query("DELETE FROM kurikulum WHERE Kurikulum_ID='$_GET[gos]'");

 header('location:../media.php?page=akademikmatakuliah&codd='.$kode.'&kode='.$jur);
}


elseif ($page=='akademikmatakuliah' AND $act=='addmk'){
        $kode= $_REQUEST['i'];
        $jur= $_REQUEST['jur'];
        $nkur= $_REQUEST['nkur'];
        $kmk= $_POST['kmk'];
...
elseif ($page=='akademikmatakuliah' AND $act=='upmk'){
        $kode= $_REQUEST['i'];
        $jur= $_REQUEST['jur'];
        $nkur= $_REQUEST['nkur'];
...
elseif ($page=='akademikmatakuliah' AND $act=='InsertPras'){
        $kode= $_REQUEST['kode'];
        $jur= $_REQUEST['jur'];
        $id= $_REQUEST['ID'];
 ...
 header('location:../media.php?page=akademikmatakuliah&PHPIdSession=prasyratMtk&gis='.$kodemk.'&kode='.$jur.'&ID='.$id);
}

elseif ($page=='akademikmatakuliah' AND $act=='delMtkPras'){
        $jur= $_REQUEST['jur'];
        $id= $_REQUEST['ID'];
        $kodemk= $_REQUEST['kodemk'];
  mysql_query("DELETE FROM mtkpra WHERE Matakuliah_ID='$_GET[kodeMtk]' AND MKPraID='$_GET[gos]'");
 header('location:../media.php?page=akademikmatakuliah&PHPIdSession=prasyratMtk&gis='.$_GET['kodeMtk'].'&kode='.$jur.'&ID='.$id);
}
